...
int CommTarget::init(const struct sockaddr *addr,
                     socklen_t addrlen,
                     int connect_timeout,
...
int CommService::init(const struct sockaddr *bind_addr,
                      socklen_t addrlen,
                      int listen_timeout,
                      int response_timeout)
{
...
int Communicator::sleep(SleepSession *session)
{
    ...
    if (session->duration(&value) >= 0) {
        ...
        if (this->poller->put_io(&data, (int) (value.tv_sec * 1000 + value.tv_nsec / 1000000))
            >= 0) {
            total_fd_cnt++;
            return 0;
        }
...
void Communicator::handle_accept_result(struct poller_result *res)
{
    ...
    case PR_ST_FINISHED:
        ...
        if (target->init((const struct sockaddr *) ctx->remote, ctx->remote_len, sockfd, service)
            >= 0) {
...
int Communicator::send_message_async(struct iovec vectors[], int cnt, CommConnEntry *entry)
{
    struct poller_data data;
...
#define PADDEDul(x, y) \
    unsigned long x; \
    unsigned y
#elif defined(__ia64__) || defined(__x86_64__) || defined(__alpha__) || defined(_M_IA64) \
    || defined(_M_AMD64) || defined(_M_ALPHA)
...
static inline int io_getevents(
